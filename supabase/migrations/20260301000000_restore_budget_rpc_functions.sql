-- Migration: Restore missing budget RPC functions
-- Date: 2026-03-01
--
-- This migration restores get_project_budget_summary_v2 and
-- get_project_expense_breakdown which are required by the frontend
-- but were missing from the active migrations (they were in archives).
-- These functions support the flexible budgeting system (project-level budget).

BEGIN;

-- 1. Restore get_project_budget_summary_v2
CREATE OR REPLACE FUNCTION public.get_project_budget_summary_v2(p_project_id UUID)
RETURNS TABLE (
...
-- 2. Restore get_project_expense_breakdown
CREATE OR REPLACE FUNCTION public.get_project_expense_breakdown(p_project_id UUID)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.get_project_budget_summary_v2(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_project_expense_breakdown(UUID) TO authenticated;

-- Add comments
COMMENT ON FUNCTION public.get_project_budget_summary_v2(UUID) IS
  'Returns overall budget summary for a project including spent, pending, and available amounts.';

COMMENT ON FUNCTION public.get_project_expense_breakdown(UUID) IS
